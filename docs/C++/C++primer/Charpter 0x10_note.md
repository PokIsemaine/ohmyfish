...
* 这两个规则暗示，我们可以将任意类型的实参传递给`T&&`类型的函数参数(T&&中&&不代表右值引用而是万能引用，多一个`const` 都不可以,而且必须是推导的用具体实例不行)

复习的时候看一下这篇资料 [现代C++之万能引用、完美转发、引用折叠(万字长文)](https://cloud.tencent.com/developer/article/1561681)
