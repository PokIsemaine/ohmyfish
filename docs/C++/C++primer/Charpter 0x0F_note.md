...
* 基类或派生类的合成拷贝控制成员除了对类本身对象一次初始化、赋值或销毁，还负责使用直接基类中对应的操作对一个对象的直接基类部分继续你初始化、赋值或销毁
...
* 定义了虚析构函数阻止了合成的移动操作，所以如果基类没有移动操作，那么派生类也没有
...
* 一个类只继承其直接基类的构造函数，类不能继承默认、拷贝和移动构造函数，如果类没有直接定义这些构造函数，则编译器将为派生类合成它们
